...
if (($tipoProducto == "01" || $tipoProducto == "02" || $tipoProducto == "04") && $idProducto != 0) {
  if ($tipoProducto == "01" && $IdFT != 0) {
    for ($i=1; $i <= sizeof($productoFichaTecnicaDet); $i++) { 
      ...
      $sqlFichaTecnicaDet = "insert into fichatecnicadet (Id, codigo, Componente, Cantidad, UnidadMedida, Costo, IdFT, Subtotal, Factor, Estado, Tipo, TipoProducto, PesoBruto, PesoNeto) values (NULL, '".$productoFichaTecnicaDet[$i]."', '".$descProductoFichaTecnicaDet."', '0', 'u', '0', '".$IdFT."', '0', '0', '0', 'Preparación', 'Preparación', '0', '0')";
