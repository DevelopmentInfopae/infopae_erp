...
$semanasMes = [];
if($semanaInicial != ''){
	$bInicial = strpos($semanaInicial, 'b');
	if ($bInicial !== false) {
		$semanaInicial = trim($semanaInicial, 'b');
	}
}
if ($semanaFinal != '') {
	$bFinal = strpos($semanaFinal, 'b');
	if ($bFinal !== false) {
		$semanaFinal = trim($semanaFinal, 'b');
	}
}

$consSemanasMes = "SELECT SEMANA FROM planilla_semanas 
					WHERE SEMANA BETWEEN '$semanaInicial' AND '$semanaFinal' 
					GROUP BY SEMANA;"; //obtenemos las semanas que hay en el mes
...
// mes de atencion 
$consultaMesAtencion = " SELECT NombreMes FROM planilla_dias WHERE mes = '$mes' ";
$respuestaMesAtencion = $Link->query($consultaMesAtencion) or die ($Link->error);
if ($respuestaMesAtencion->num_rows > 0) {
	$dataMesAtencion = $respuestaMesAtencion->fetch_assoc();
	$mesAtencion = $dataMesAtencion['NombreMes'];
}

//*/*/*/*/*/*/*/*/*/*/*/*/*/*/*/*/*/*/*/*/*/*/*/*/*/*/*/*/*/*/*/*/*/*/*/*/*/*/*/*/*/*/*/*/*/*/*/*/*/*/*/*/*/*/*/*/*/*/
// modificacion putumayo para poder imprimir cuando van combinalos meses
...
if ($tipoPlanilla != 6) {
	foreach ($semanasMes as $semana => $set) { 
		...
		$consTotalEntregado = "SELECT DISTINCT SEMANA AS SM, 
												MES AS MS, 
												COUNT(DIA) AS dias_semana, 
												((SELECT SUM(".$tipoComplemento.") FROM priorizacion".$semana." WHERE cod_sede = '".$codigoSede."') * (SELECT COUNT(DIA) FROM planilla_semanas WHERE SEMANA = SM AND MES = MS)) AS total_entregas
...
$pdf->SetXY($x, $y);
$pdf->Cell(10,14,utf8_decode(''),'RB',0,'C',False);
...
$pdf->SetXY($x, $y);
$pdf->Cell(22,14,utf8_decode(''),'RB',0,'C',False);
...
$pdf->SetXY($x, $y);
$pdf->Cell($anchoDatosNombre,14,utf8_decode(''),'RB',0,'C',False);
...
if ($tipoPlanilla == 1 || $tipoPlanilla == 2 || $tipoPlanilla == 3 || $tipoPlanilla == 4) {
	...
	$y = $pdf->GetY();
	$pdf->SetXY($x, $y);
	$pdf->RotatedText($x+3.5,$y+12,utf8_decode("P. Étnica"),90);
	$pdf->Cell(5,14,utf8_decode(''),'RB',0,'C',False);
...
// $pdf->RotatedText($x+6,$y+13,utf8_decode('Educación'),90);
$pdf->Cell(8,14,utf8_decode(''),'RB',0,'C',False);
...
$pdf->SetXY($x, $y);
$pdf->Cell(13,14,utf8_decode(''),'RB',0,'C',False);
...
if($tipoPlanilla != 1 ){
	...
	for($i = 0 ; $i < 25 ; $i++){
			...
			if(isset($dias_encabezado[$auxDia])){
		 		$pdf->Cell(6,7,utf8_decode($dias_encabezado[$auxDia]),'RB',0,'C',False);
			}else{
		 		$pdf->Cell(6,7,"",'RB',0,'C',False);
			}
		$dia++;
	}
}else{
...
$pdf->SetXY($x, $y);
$pdf->Cell(0,7,utf8_decode(''),'B',0,'C',False);
