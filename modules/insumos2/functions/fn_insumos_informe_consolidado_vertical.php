...
if ($cantGruposEtarios == 3) {
	...
	class PDF extends FPDF{
		function setData($fecha, $dpto, $dataSede, $coberturaEtarios, $maxEstudiantes, $gruposEtarios, $mes, $tipoComplemento, $entrega, $Rpc, $contrato, $numManipuladoras, $nombreMesTabla, $zona){
			...
			$this->tipoComplemento = substr($tipoComplemento, 0, 10);
...
if ($cantGruposEtarios == 5) {
	...
	class PDF extends FPDF{
		function setData($fecha, $dpto, $dataSede, $coberturaEtarios, $maxEstudiantes, $gruposEtarios, $mes, $tipoComplemento, $entrega, $Rpc, $contrato, $numManipuladoras, $nombreMesTabla, $zona){
			...
			$this->contrato = $contrato;
			$this->manipuladoras = $numManipuladoras;
			$this->nombreMesTabla = substr($nombreMesTabla, 0,30) ;
		}
		...
		function Header() {
			...
			$this->Cell(19,4,utf8_decode('CONTRATO:'),'B',0,'L');
			$this->SetFont('Arial','',8);
			$this->Cell(20,4,utf8_decode($this->contrato),'BR',0,'L'); 
			$this->SetFont('Arial','B',8);
			$this->Cell(9,4,utf8_decode('MES:'),'B',0,'L');
			$this->SetFont('Arial','',8);
			$this->Cell(0,4,utf8_decode($this->nombreMesTabla), 'BR', 1, 'L'); //FECHA DE ELABORACIÓN DEL DESPACHO - REEMPLAZAR	
			...
			$this->setXY($cx+(35*2+40), $cy);
			$this->Cell(34, 30, utf8_decode($this->manipuladoras), 'TBLR', 0, 'C');//MANIPULADORAS DE LA SEDE
			$this->Cell(32, 30, utf8_decode($this->maxEstudiantes), 'TBR', 0, 'C');//CANTIDAD COBERTURA
			$this->Cell(25.9, 30, utf8_decode($this->tipoComplemento), 'TBR', 0, 'C', FALSE);//CANTIDAD COBERTURA
	...
	$coberturaEtarios["Etario2"] = 0;
	$coberturaEtarios["Etario3"] = 0;
	$coberturaEtarios["Etario4"] = 0;
	$coberturaEtarios["Etario5"] = 0;
	$maxEstudiantes = 0;
	$numeroIn = '(';
	$sedesIn = '(';
