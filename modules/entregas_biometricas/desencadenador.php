...
$consulta = " SELECT f.nom1, f.nom2, f.ape1, f.ape2, f.telefono, f.edad, f.Tipo_complemento FROM focalizacion18 f LEFT JOIN biometria b ON b.tipo_doc = f.tipo_doc AND b.num_doc = f.num_doc WHERE b.id_dispositivo = \"$dispositivoId\" AND b.id_bioest = $usrDispositivo ";
//echo "<br>$consulta<br>";
...
/* Menu del día */
$consulta = " SELECT ps.SEMANA, ps.DIA, p.Id, p.Codigo, p.Descripcion FROM planilla_semanas ps LEFT JOIN productos19 p ON ps.MENU = p.Orden_Ciclo WHERE ps.MES = \"$mes\" AND ps.DIA = \"$dia\" AND p.Cod_Tipo_complemento = \"$complemento\" AND p.Cod_Grupo_Etario = \"$grupoEtario\" AND p.Codigo LIKE \"01%\" AND p.Nivel = 3 ";
//echo "<br>$consulta<br>";
...
if($resultado->num_rows >= 1){
	while($row = $resultado->fetch_assoc()){
		$preparaciones .= $row["Componente"]."\n";
	}
}
//echo $preparaciones;
$mensaje = " Hola,\n\n $nombre recibió el complemento $complemento: \n\n ";
...
$message = $twilio->messages 
                  ->create("whatsapp:+573166170054", // to 
                           array( 
