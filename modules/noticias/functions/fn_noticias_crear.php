<?php
    ...
    if (!isset($_FILES["imagen"])) {
        echo json_encode([
            "estado"=>0,
            "mensaje"=>"Debe agregar una imagen para continuar."
        ]);
        exit();
    } else {
        $consulta = "INSERT INTO noticias (fecha, titulo, descripcion, usuario_registro) VALUES ('". $fecha ."', '". $titulo ."', '". $descripcion ."', '". $_SESSION["id_usuario"] ."')";
        $respuesta = $Link->query($consulta) or die ("Error al crear el registro de noticia: ". $Link->error);
        ...
        if ($respuesta) {
            ...
            echo json_encode([
                "estado" => 1,
                "mensaje" => "La noticia han sido guardada exitosamente."
            ]);
        }
    }
