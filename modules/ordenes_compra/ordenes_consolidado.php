...
$rowNum++;
$sheet->setCellValue('I'.$rowNum, 'CONSOLIDADO: REMISIÓN ENTREGA DE VÍVERES POR MUNICIPIO');
$sheet->mergeCells('I'.$rowNum.':R'.$rowNum);
$rowNum++;
...
$sheet->setCellValue('C'.($rowNum+1), $etario_1." años");
$sheet->setCellValue('D'.($rowNum+1), $etario_2." años");
$sheet->setCellValue('E'.($rowNum+1), $etario_3." años");

$sheet->setCellValue('F'.$rowNum, 'UNIDAD DE MEDIDA');
$sheet->mergeCells('F'.$rowNum.':F'.($rowNum+1));
$sheet->setCellValue('G'.$rowNum, 'CNT TOTAL');
   ...
   for ($i=0; $i < count($alimentosTotales ) ; $i++) {
    ...
    if($item['presentacion'] == 'u'){
      ...
      $aux = number_format($aux, 2, '.', '');
        ...
        $sheet->setCellValue('F'.$rowNum, $item['presentacion']);
...
header('Content-Disposition: attachment; filename="ordenes_consolidado.xlsx"');
$writer->save('php://output','ordenes_consolidado.xlsx');
