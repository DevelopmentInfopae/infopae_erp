...
foreach ($despachosRecibidos as &$valor){
  //echo "<br>".$valor."<br>";
  $consulta = " select de.*, tc.descripcion , u.Ciudad, tc.jornada, s.nom_inst , s.nom_sede from despachos_enc$mesAnno de inner join sedes$anno  s on de.cod_Sede = s.cod_sede inner join ubicacion u on s.cod_mun_sede = u.CodigoDANE left join tipo_complemento tc on de.Tipo_Complem = tc.CODIGO where Tipo_Doc = 'DES' and de.Num_Doc = $valor ";
  //echo "<br>$consulta<br>";
...
// La firma mide aprox 72 mm
if((215.9 - $current_y) < 72){
	$pdf->AddPage();
	include 'despacho_kardex4_multiple_header.php';
	include 'despacho_firma_planilla_kardex2017.php';
}else{
	include 'despacho_firma_planilla_kardex2017.php';
}
$pdf->Output();
