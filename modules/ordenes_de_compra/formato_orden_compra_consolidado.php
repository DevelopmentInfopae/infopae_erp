...
if ($cantGruposEtarios == '3') {
	...
	$consultaCoberturas = " SELECT 	SUM(Cobertura_G1) AS Cobertura_G1, 
												SUM(Cobertura_G2) AS Cobertura_G2, 
												SUM(Cobertura_G3) AS Cobertura_G3, 
												( SUM(Cobertura_G1) + SUM(Cobertura_G2) + SUM(Cobertura_G3) ) AS sumaCoberturas 
											FROM orden_compra_enc$mesAnno 
											WHERE Num_OCO IN ($ordenesCompra)  ";
...
if ($cantGruposEtarios == '5') {
	...
	$ciclos = array_unique ($ciclos);
	sort($ciclos);
	$auxDias = "X ".$cantDias." DIAS ".$auxDias." de ".$mes;
	$auxDias = strtoupper($auxDias);
	$auxSemana = '';
	for ($i=0; $i < count($semanasMostrar) ; $i++) {
		if($i > 0){ $auxSemana = $auxSemana.", "; }
	...
	$consultaCoberturas = " SELECT 	SUM(Cobertura_G1) AS Cobertura_G1, 
												SUM(Cobertura_G2) AS Cobertura_G2, 
												SUM(Cobertura_G3) AS Cobertura_G3, 
												SUM(Cobertura_G4) AS Cobertura_G4, 
												SUM(Cobertura_G5) AS Cobertura_G5, 
												( SUM(Cobertura_G1) + SUM(Cobertura_G2) + SUM(Cobertura_G3) + SUM(Cobertura_G4) + SUM(Cobertura_G5)) AS sumaCoberturas 
											FROM orden_compra_enc$mesAnno 
											WHERE Num_OCO IN ($ordenesCompra)  ";
	...
	foreach ($grupos_alimentarios as $nombre_grupo => $grupo_alimentario){
		...
		foreach ($grupo_alimentario as $alimento){
			...
			$pdf->Cell(13.1, 4, $alimento['presentacion'], 'LB', 0, 'C', FALSE);
			...
			$pdf->Cell(13.1, 4, number_format($aux, 2, '.', ''), 'LB', 0, 'C', FALSE);
			if ($alimento['redondeo'] == 1) {
				if ($aux <= 0.5) {
					$aux = ceil($aux);
				}else if ($aux > 0.5) {
			...
			$pdf->Cell(11, 4, $aux, 'LB', 0, 'C', FALSE);

			//total entregado
			$pdf->Cell(10, 4, '', 'LB', 0, 'C', FALSE);
			$pdf->Cell(10, 4, '', 'LB', 0, 'C', FALSE);
