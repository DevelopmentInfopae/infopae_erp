...
foreach ($focexiste as $foc) {
	$sqldesactivar = "UPDATE ".$foc." SET activo = 0 WHERE num_doc = ".$num_doc."; ";
	if ($Link->query($sqldesactivar)===true) {
		$cnt++;
	}
}

if ($cnt == sizeof($focexiste)) {
    $sqlBitacora = "INSERT INTO bitacora (id, fecha, usuario, tipo_accion, observacion) VALUES ('', '".date('Y-m-d H:i:s')."', '".$_SESSION['idUsuario']."', '49', 'Desactivó al títular de derecho con número de identificación <strong>".$num_doc."</strong>')";
