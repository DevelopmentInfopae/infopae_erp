<?php
	...
	$rango = (isset($_POST['rango']) && $_POST['rango'] != '') ? mysqli_real_escape_string($Link, $_POST["rango"]) : "";
	...
	elseif ($rango == 0) {
		$consulta_priorizacion = "SELECT
							    			SUM($tipo_complemento * (SELECT COUNT(DIA) FROM planilla_semanas WHERE MES='$mes' AND SEMANA='$semana')) AS total_priorizacion
										FROM
							    			priorizacion$semana
										WHERE
							    			cod_sede like '$institucion%'";
