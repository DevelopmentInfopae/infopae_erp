...
if ($cantGruposEtarios == 3) {   
   ...
   $pdf->Cell(146.45,5.7,utf8_decode('REMISION DE ENTREGA A SEDES EDUCATIVAS ATENCIÓN EN EL MARCO DE EMERGENCIA SANITARIA COVID19'),0,5.7,'C',False);
...
if ($cantGruposEtarios == 5) {
   ...
   if($imprimirMes == 0){ $auxDias = ""; }
   $pdf->Cell(45,23.5,$auxDias,1,0,'C',False);
   ...
   $pdf->Cell(57.8, 23.5, 'N/A', 'BR', 0, 'C', False);
   $pdf->SetXY($current_x, $current_y+2.35);
   ...
   $aux = $totalGrupo1 + $totalGrupo2 + $totalGrupo3 + $totalGrupo4 + $totalGrupo5;
   $pdf->Cell(0, 23.5, $aux, 'BR', 0, 'C', False);
   ...
   $pdf->Cell(11, 15, '', 1, 0, 'L', False);
   $pdf->SetXY($current_x, $current_y);
   $pdf->Cell(11, 5, 'UNIDAD', 0, 5, 'C', False);
   $pdf->Cell(11, 5, 'DE', 0, 5, 'C', False);
   $pdf->Cell(11, 5, 'MEDIDA', 0, 5, 'C', False);
   $pdf->SetXY($current_x+11, $current_y);

   $current_y = $pdf->GetY();
   $current_x = $pdf->GetX();
   $pdf->Cell(11, 15, '', 1, 0, 'L', False);
   $pdf->SetXY($current_x, $current_y+2.5);
   $pdf->Cell(11, 5, 'CNT', 0, 5, 'C', False);
   $pdf->Cell(11, 5, 'TOTAL', 0, 5, 'C', False);
   $pdf->SetXY($current_x+11, $current_y);
   
   $current_y = $pdf->GetY();
   $current_x = $pdf->GetX();
   $pdf->Cell(29, 15, '', 1, 0, 'L', False);
   $pdf->SetXY($current_x, $current_y);
   $pdf->Cell(29, 4, 'CANTIDAD', 0, 4, 'C', False);
   $pdf->Cell(29, 4, 'ENTREGADA', 'B', 4, 'C', False);
   $pdf->Cell(11, 7, 'TOTAL', 'R', 0, 'C', False);
   $pdf->Cell(9, 7, 'C', 'R', 0, 'C', False);
   $pdf->Cell(9, 7, 'NC', 'R', 0, 'C', False);

   $pdf->SetXY($current_x+29, $current_y);
   $current_y = $pdf->GetY();
   $current_x = $pdf->GetX();
   $pdf->Cell(20, 15, '', 1, 0, 'L', False);
   $pdf->SetXY($current_x, $current_y);
   $pdf->Cell(20, 4, utf8_decode('ESPECIFICACIÓN'), 0, 4, 'C', False);
   $pdf->Cell(20, 4, utf8_decode('DE CALIDAD'), 'B', 4, 'C', False);
   $pdf->Cell(10,7,'C','R',0,'C',False);
   $pdf->Cell(10,7,'NC','R',0,'C',False);
   ...
   $pdf->SetXY($current_x, $current_y);
   $pdf->Cell(27, 8, utf8_decode('FALTANTES'), 'B', 8, 'C', False);
   $pdf->Cell(8, 7, 'SI', 'R', 0, 'C', False);
   $pdf->Cell(8, 7, 'NO', 'R', 0, 'C', False);
   $pdf->Cell(11, 7, 'CANT', 'R', 0, 'C', False);
