...
foreach ($codesedes as $sedecod => $isset) {
            ...
            if(!in_array($row['Semana'], $semanasMostrar, true)) {
                ...
                if($resultado->num_rows >= 1) {
                    while($datos_semanas = $resultado->fetch_assoc()) {
                        $mes = $datos_semanas['MES'];
                        $ciclo = $datos_semanas['CICLO'];
                        $ciclos[$ciclo] = $ciclo;
    ...
    include 'despacho_kardex_multiple_vertical_header.php';
    ...
    foreach ($alimentos_por_grupos_alimentos as $nombre_grupo_alimenticio => $alimentos) {
        $altura_celda_grupo_alimenticio = count($alimentos);
        $altura_pagina = (isset($posicion_Y_celda_alimento) ? $posicion_Y_celda_alimento : 0) + $altura_celda_grupo_alimenticio;
        if ($altura_pagina > 280) {
            $pdf->AddPage();
            include 'despacho_kardex_multiple_vertical_header.php';
        }
        $posicion_Y_celda_grupo_alimenticio = $pdf->GetY();
        foreach ($alimentos as $alimento) {
            $pdf->Cell(45.2, 4, utf8_decode(substr($alimento['componente'], 0, 36)), 1, 0, 'L');

            // Unidad de medida
            $pdf->Cell(13, 4, utf8_decode($alimento['presentacion']), 1, 0, 'C');
            ...
            for($k = 1; $k <= 5; $k++) {
                ...
                if ($alimento['presentacion'] == "u") {
                    ...
                    $pdf->Cell(18, 4, $consumoDia, '1', 0, 'C');
                }
                else {
                    $consumoDia = number_format($consumoDia, $digitosDecimales);
                    $pdf->Cell(18, 4, $consumoDia, '1', 0, 'C');
    ...
    if($current_y > 230) {
       $pdf->AddPage();
       include 'despacho_kardex_multiple_vertical_header.php';
       include 'despacho_kardex_multiple_vertical_firma.php';
    } else {
       include 'despacho_kardex_multiple_vertical_firma.php';
    }
}
