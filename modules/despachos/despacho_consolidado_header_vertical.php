...
if ($cantGruposEtarios == 5) {
	...
	$pdf->Cell(35, 21.5, utf8_decode(''), 'R', 0, 'C', False);
	$pdf->Cell(22, 21.5, utf8_decode(''), 'R', 0, 'C', False);

	$auxDias = "X ".$cantDias." DIAS ".strtoupper($dias)."\n"."SEMANA: ".$semana;
	$aux_y_antes = $pdf->GetY();
	$aux_x_antes = $pdf->GetX();
	$pdf->Cell(27, 21.5, '', 'R', 0, 'C', False);

	$aux_y_despues = $pdf->GetY();
	$aux_x_despues = $pdf->GetX();
	$pdf->SetXY($aux_x_antes, $aux_y_antes+0.5);
	$pdf->MultiCell(26, 7, utf8_decode($auxDias), 0, 'C', False);
	$pdf->SetXY($aux_x_despues, $aux_y_despues);
	$pdf->Cell(16, 21.5, utf8_decode($auxMenus), 'R', 0, 'C', False);
	$pdf->Cell(18, 21.5, utf8_decode($ciclo), 'R', 0, 'C', False);
	...
	$pdf->Cell(14, 21.5, utf8_decode($jm), 'R', 0, 'C', False);
	$pdf->Cell(0, 21.5, utf8_decode($jt), 0, 0, 'C', False);
	...
	$pdf->Cell(35, 4.3, utf8_decode($totalGrupo1), 'B', 5, 'C', False);
	$pdf->Cell(35, 4.3, utf8_decode($totalGrupo2), 'B', 5, 'C', False);
	$pdf->Cell(35, 4.3, utf8_decode($totalGrupo3), 'B', 5, 'C', False);
	$pdf->Cell(35, 4.3, utf8_decode($totalGrupo4), 'B', 5, 'C', False);
	$pdf->Cell(35, 4.3, utf8_decode($totalGrupo5), 0, 0, 'C', False);

	$pdf->SetXY($current_x+34+35, $current_y);
	$pdf->Cell(22, 4.3, utf8_decode($totalGrupo1), 'B', 5, 'C', False);
	$pdf->Cell(22, 4.3, utf8_decode($totalGrupo2), 'B', 5, 'C', False);
	$pdf->Cell(22, 4.3, utf8_decode($totalGrupo3), 'B', 5, 'C', False);
	$pdf->Cell(22, 4.3, utf8_decode($totalGrupo4), 'B', 5, 'C', False);
	$pdf->Cell(22, 4.3, utf8_decode($totalGrupo5), 0, 0, 'C', False);
	...
	$pdf->SetXY($current_x, $current_y);
	$pdf->Cell(34,10,utf8_decode(''),'LTB',0,'C',true);
	$pdf->Cell(57.5,10,utf8_decode(''),'LTB',0,'C',true);
	$pdf->Cell(11,10,utf8_decode(''),'LTB',0,'C',true);
	$pdf->Cell(11,10,utf8_decode(''),'LTB',0,'C',true);
	$pdf->Cell(21,10,utf8_decode(''),'LTB',0,'C',true);
	$pdf->Cell(17,10,utf8_decode(''),'LTB',0,'C',true);
	$pdf->Cell(20,10,utf8_decode(''),'LTB',0,'C',true);
	$pdf->Cell(0,10,utf8_decode(''),1,0,'C',true);
	...
	$aux_y_antes = $pdf->GetY();
	$aux_x_antes = $pdf->GetX();
	$pdf->Cell(57.5, 5, utf8_decode(''), 'B', 0, 'C', False);
	$pdf->SetXY($aux_x_antes, $aux_y_antes);
	$pdf->MultiCell(57.5, 2.5, utf8_decode('CANTIDAD DE ALIMENTOS POR NÚMERO DE RACIONES'), 0, 'C', false);

	$pdf->SetXY($aux_x_antes, $aux_y_antes+5);
	$pdf->Cell(11.5, 5, utf8_decode(''), 'R', 0, 'C', false);
	$pdf->Cell(11.5, 5, utf8_decode(''), 'R', 0, 'C', false);
	$pdf->Cell(11.5, 5, utf8_decode(''), 'R', 0, 'C', false);
	$pdf->Cell(11.5, 5, utf8_decode(''), 'R', 0, 'C', false);
	...
	$pdf->SetFont('Arial','B',$tamannoFuente);
	$pdf->SetXY($aux_x_antes, $aux_y_antes+5.3);
	$pdf->MultiCell(11.5, 1.5, utf8_decode($get[0]), 0, 'C', false);
	$pdf->SetXY($aux_x_antes+11.5, $aux_y_antes+5.3);
	$pdf->MultiCell(11.5,1.5,utf8_decode($get[1]),0,'C',false);
	$pdf->SetXY($aux_x_antes+11.5+11.5, $aux_y_antes+5.3);
	$pdf->MultiCell(11.5,1.5,utf8_decode($get[2]),0,'C',false);
	$pdf->SetXY($aux_x_antes+11.5+11.5+11.5, $aux_y_antes+5.3);
	$pdf->MultiCell(11.5,1.5,utf8_decode($get[3]),0,'C',false);
	$pdf->SetXY($aux_x_antes+11.5+11.5+11.5+11.5, $aux_y_antes+5.3);
	$pdf->MultiCell(11.5, 1.5, utf8_decode($get[4]), 0, 'C', false);

	$pdf->SetXY($current_x+34+57.5, $current_y+2);
	$pdf->MultiCell(11,2,utf8_decode('UNIDAD DE MEDIDA'),'R','C',false);
	$pdf->SetXY($current_x+34+57.5+10.5, $current_y+3);
	$pdf->MultiCell(12,2,utf8_decode('CANTIDAD TOTAL'),0,'C',false);

	$pdf->SetXY($current_x+34+57.5+11+11, $current_y);
	$current_y = $pdf->GetY();
	$current_x = $pdf->GetX();
	$pdf->Cell(21, 5, utf8_decode('CANTIDAD ENTREGADA'), 'B', 0, 'C', False);
	$pdf->SetXY($current_x, $current_y+5);
	$pdf->Cell(9, 5, utf8_decode('TOTAL'), 'R', 0, 'C', False);
	$pdf->Cell(6, 5, utf8_decode('C'), 'R', 0, 'C', False);
	$pdf->Cell(6, 5, utf8_decode('NC'), 0, 0, 'C', False);

	$pdf->SetXY($current_x+21, $current_y);
	$current_y = $pdf->GetY();
	$current_x = $pdf->GetX();
	$pdf->Cell(17,5,utf8_decode(''),'B',0,'C',False);
	$pdf->SetXY($current_x, $current_y);
