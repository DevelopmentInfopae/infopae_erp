...
*/
  ...
  include 'despacho_mixta_header.php';
  ...
  for ($i=0; $i < count($alimentos ) ; $i++) {
    ...
    if($alimento['grupo_alim'] != $grupoAlimActual){
        ...
        if($alimento['cantu2'] > 0){
          $filas++;
        }
        if($alimento['cantu3'] > 0){
          $filas++;
        }
        if($alimento['cantu5'] > 0){
          $filas++;
        }
        if($alimento['cantu4'] > 0){
          $filas++;
        }

        // Mirar si caben todas las filas del grupo.
        if(($current_y + (4*$filas)) > 205){
          $pdf->AddPage();
          include 'despacho_por_sede_footer.php';
          include 'despacho_mixta_header.php';
          $pdf->SetFont('Arial','',$tamannoFuente);
        ...
        $current_x = $pdf->GetX();
        $altura = 4 * $filas;
        $pdf->Cell(23.788,$altura,'',1,0,'C',False);
