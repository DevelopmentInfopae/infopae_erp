...
for ($kDespachos=0; $kDespachos < count($_POST) ; $kDespachos++) {
  ...
  $resultado = $Link->query($consulta) or die ('Unable to execute query 1. '. mysqli_error($Link));
  ...
  $resultado = $Link->query($consulta) or die ('Unable to execute query 2. '. mysqli_error($Link));
  ...
  $resultado = $Link->query($consulta) or die ('Unable to execute query 3. Sedes cobertura '. mysqli_error($Link));
  if($resultado->num_rows >= 1){
    $row = $resultado->fetch_assoc();
...
$resultado = $Link->query($consulta) or die ('Unable to execute query 4 codigo de alimento. '. mysqli_error($Link));
if($resultado->num_rows >= 1){
...
*/
  ...
  for ($i=0; $i < count($alimentos) ; $i++) {
    ...
    (SELECT cantu2 FROM despachos_det$mesAnno WHERE Tipo_Doc = 'DES' AND Num_Doc = $despacho AND cod_Alimento = $auxCodigo limit 1 ) AS cantu2,
    (SELECT cantu3 FROM despachos_det$mesAnno WHERE Tipo_Doc = 'DES' AND Num_Doc = $despacho AND cod_Alimento = $auxCodigo limit 1 ) AS cantu3,
    (SELECT cantu4 FROM despachos_det$mesAnno WHERE Tipo_Doc = 'DES' AND Num_Doc = $despacho AND cod_Alimento = $auxCodigo limit 1 ) AS cantu4,
    (SELECT cantu5 FROM despachos_det$mesAnno WHERE Tipo_Doc = 'DES' AND Num_Doc = $despacho AND cod_Alimento = $auxCodigo limit 1 ) AS cantu5,
    (SELECT cantotalpresentacion FROM despachos_det$mesAnno WHERE Tipo_Doc = 'DES' AND Num_Doc = $despacho AND cod_Alimento = $auxCodigo limit 1 ) AS cantotalpresentacion,
    ...
    $resultado = $Link->query($consulta) or die ('Unable to execute query 5. detalles de alimentos'. mysqli_error($Link));
  ...
  include 'despacho_kardex_header.php';
 ...
 $b = number_format($b, $digitosDecimales);
    ...
    //En este informe no se muestra el total requerido, solo el despachado.
    ...
    for( $k = 1; $k <= 5; $k++ ){
        ...
        $pdf->Cell(12,4,$consumoDia,'1',0,'C',False);
        $pdf->Cell(12,4,$saldo,'1',0,'C',False);
        $current_y = $pdf->GetY();
        $current_x = $pdf->GetX();
        $pdf->Cell(8,4,'','1',0,'C',False);
        $pdf->SetXY($current_x+2.5, $current_y+0.5);
        $pdf->Cell(3,3,'','1',0,'C',False);
        $pdf->SetXY($current_x+8, $current_y);
    }
