<?php
  ...
  if(isset($_POST['itemsDespachoVariaciones']) && $_POST['itemsDespachoVariaciones'] != ''){
    ...
    // exit(json_encode($sedes_variaciones));
...
for ($i=0; $i < count($sedes) ; $i++) { 
  ...
  $sede_turno = $sedes[$i];
  $variacion_sede = $sedes_variaciones[$sede_turno];
    ...
    // Primera consulta, la que trae los diferentes alimentos de los menu.
      ...
      and p.Cod_Tipo_complemento = '$tipo' AND p.cod_variacion_menu = '$variacion_sede' ";
    ...
    // exit(json_encode($sedeCobertura));
    ...
    if($bandera == 0 && $cantidadItems > 0){
      ...
      //for ($i=0; $i < count($sedes); $i++){
        $bodegaDestino = $sede_turno;
        //$consecutivo = $consecutivos[0];
      ...
      //CONTINUAR ACÁ, VALIDAR SI PRIMERA CONSULTA SE TRAE LOS PRODUCTOS CORRESPONDIENTES
      for ($i=0; $i < count($sedesCobertura) ; $i++) {
        $auxItem = 1;
        for ($j=0; $j < count($complementosCantidades) ; $j++){
          $auxConsulta = '';
      ...
      $resultado = $Link->query($consulta) or die ('Unable to execute query - Inserción en productosmovdet '. mysqli_error($Link)."consulta </br> ".$consulta." </br> Complementos : " .json_encode($complementosCantidades)));
