...
if ($cantGruposEtarios == '3') {
	...
	// Se va a hacer una cossulta pare obtener los datos de cada movimiento, entre ellos el municipio que lo usaremos en los encabezados de la tabla.
	$mes = '';
	$sede = '';
	$dias = '';
	$ciclo = '';
	...
	foreach ($despachosRecibidos as &$valor){
		...
		if($resultado->num_rows >= 1){
			...
			// Agregando elementos a los demas 
			$sedes[] = $row['cod_Sede'];
	...
	foreach ($sede_unicas as $key => $sede_unica) {
		...
		$consulta = "SELECT 	DISTINCT 	max(Cobertura_G1) AS Cobertura_G1, 
													...
													cod_sede, 
													(max(Cobertura_G1) + max(Cobertura_G2) + max(Cobertura_G3)) AS sumaCoberturas 
	...
	for ($i=0; $i < count($despachos) ; $i++) {
		...
		$consulta = " SELECT DISTINCT 	dd.id, dd.*, 
								...
								LEFT JOIN productos$anno p ON dd.cod_Alimento = p.Codigo WHERE dd.Tipo_Doc = 'DES' AND dd.Num_Doc = $numero ";
...
if ($cantGruposEtarios == '5') {
	...
	foreach ($sede_unicas as $key => $sede_unica) {
		...
		$consulta = "SELECT 	DISTINCT 	max(Cobertura_G1) AS Cobertura_G1, 
													...
													cod_sede, 
													(max(Cobertura_G1) + max(Cobertura_G2) + max(Cobertura_G3) + max(Cobertura_G4) + max(Cobertura_G5)) AS sumaCoberturas 
												FROM despachos_enc$mesAnno 
												WHERE estado != 0 AND semana in ($semana) AND cod_sede = $auxSede AND Tipo_Complem = '". $tipo ."' 
												ORDER BY sumaCoberturas DESC 
												LIMIT 1";
	...
	foreach ($grupos_alimentarios as $nombre_grupo => $grupo_alimentario){
		...
		foreach ($grupo_alimentario as $alimento){
			...
			$pdf->Cell(12, 4, $alimento['presentacion'], 1, 0, 'C', FALSE);
			...
			$pdf->Cell(12, 4, number_format($aux, 2, '.', ''), 1, 0, 'C', FALSE);
