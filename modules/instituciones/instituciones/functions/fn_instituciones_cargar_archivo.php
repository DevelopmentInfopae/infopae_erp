	...
	if($tipoArchivo == "vnd.ms-excel"){
		...
		while(($datos = fgetcsv($archivo, null, $separador))==true){
			...
			if (in_array($datos[2], $arrayMunicipios) === false) {
				$result = array(
					"estado" => 0, 
					"mensaje" => "El código del munipio no esta comprendido en los municipios del contrato en la fila N° ".($fila+1)."."
				);
				exit(json_encode($result));
			}
			if ($datos[2] == " ") {
				$result = array(
					"estado" => 0, 
					"mensaje" => "El codigo del municipio no puede estar vacío en la fila N° ".($fila+1)."."
				);
				exit(json_encode($result));
			}
			...
			foreach ($cellIterator as $celda) {
				if(!is_null($celda)){
					...
					if(in_array($valor3, $arrayMunicipios) === false){
					   $result = array(
							"estado" => 0, 
							"mensaje" => "El código del munipio no esta comprendido en los municipios del contrato en la fila N° ".($filaC+1)."."
						);
						echo json_encode($result);
					  	exit();
					}
