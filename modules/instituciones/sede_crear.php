          	...
          	<div class="row">
                  ...
                  <div class="form-group col-sm-3">
                    ...
                    <select class="form-control" name="institucion" id="institucion" required="">
                      <option value="">Seleccione uno</option>
                      <?php
                        $consulta = " SELECT * FROM instituciones WHERE cod_mun = '". $registrosIntitucion["cod_mun"] ."' ORDER BY nom_inst ASC";
                        $resultado = $Link->query($consulta);
                        if($resultado->num_rows > 0){
                          while($row = $resultado->fetch_assoc()) {
                            $selected = (isset($registrosIntitucion["codigo_inst"]) && $registrosIntitucion["codigo_inst"] == $row["codigo_inst"] ) ? " selected " : "";
                            echo '<option value="' . $row["codigo_inst"] . '" ' . $selected . '>
                                    ' . $row["nom_inst"] .
                                  '</option>';
                          }
                        }
                  ...
                  <div class="form-group col-sm-3">
		              ...
		              <div class="form-group col-lg-3">
			                ...
			                <div class="radio">
											  <label>
											    <input type="radio" name="validacion" id="validacion1" value="Planilla" checked required> Planilla
											  </label>
											  <label>
											    <input type="radio" name="validacion" id="validacion2" value="Lector Biométrico" required> Lector Biométrico
											  </label>
											</div>
