...
?>		<script type="text/javascript">
			...
			const list2 = document.querySelector(".li_sedes");
  	...
  	<div class="col-lg-8">
    	...
    	<ol class="breadcrumb">
      		...
      		<?php if ($_SESSION['perfil'] != 6 && $_SESSION['perfil'] != 7): ?> 
      			<li>
      				<a href="<?= $baseUrl . '/modules/instituciones/sedes.php'; ?>"><?= $titulo ?></a>
      			</li>
      		<?php endif ?>
      		<li class="active">
          		<strong>Sede</strong>
  							...
  							<table class="table table-striped">
  								...
  								<tbody>
  									<tr>
  										...
  										<?php foreach ($complementos as $key => $value): ?>
											<td><?= $datos_sede['Manipuladora_'.$value->CODIGO]; ?></td>
  										<?php endforeach ?>
        		...
        		<div class="ibox-content">
					<?php
						...
						foreach ($complementos as $key => $value) {
							$totalesPriorizacion[$value->CODIGO] = 0;
						}

						for ($i=1; $i <= $cantGruposEtarios ; $i++) {
							foreach ($complementos as $key => $value) {
								$totalesPriorizacion['Etario'.$i.'_'.$value->CODIGO] = 0;
							}
						}
						...
						foreach ($semanas as $semana) {
							...
							if($resultado->num_rows >= 1) {
								while($row = $resultado->fetch_assoc()) {
									foreach ($complementos as $key => $value) {
										if (isset($priorizacion[$value->CODIGO][$semana]) && floatval($priorizacion[$value->CODIGO][$semana]) > 0 ) {
											$priorizacion[$value->CODIGO][$semana] = $priorizacion[$value->CODIGO][$semana] + $row[$value->CODIGO];
										}else{
											$priorizacion[$value->CODIGO][$semana] = $row[$value->CODIGO];
										}
									}

									for ($i=1; $i <= $cantGruposEtarios ; $i++) {
										foreach ($complementos as $key => $value) {
											if (isset($priorizacion['Etario'.$i.'_'.$value->CODIGO][$semana]) && floatval($priorizacion['Etario'.$i.'_'.$value->CODIGO][$semana]) > 0 ) {
												$priorizacion['Etario'.$i.'_'.$value->CODIGO][$semana] = $priorizacion['Etario'.$i.'_'.$value->CODIGO][$semana] + $row['Etario'.$i.'_'.$value->CODIGO];
											} else {
												$priorizacion['Etario'.$i.'_'.$value->CODIGO][$semana] = $row['Etario'.$i.'_'.$value->CODIGO];
											}
										}
									}

									foreach ($complementos as $key => $value) {
										$totalesPriorizacion[$value->CODIGO] = $totalesPriorizacion[$value->CODIGO] + $row[$value->CODIGO];
									}

									for ($i=1; $i <= $cantGruposEtarios ; $i++) {
										foreach ($complementos as $key => $value) {
											$totalesPriorizacion['Etario'.$i.'_'.$value->CODIGO] = $totalesPriorizacion['Etario'.$i.'_'.$value->CODIGO] + $row['Etario'.$i.'_'.$value->CODIGO];
										}
									}
								}
							}
							...
							<tbody>
								<?php foreach ($complementos as $key => $value): ?>
									<?php for ($i=1; $i <= $cantGruposEtarios ; $i++) {  ?>
										<tr>
											<td>Etario <?= $i; ?> <?= $value->CODIGO; ?></td>
												<?php foreach ($semanas as $semana){ ?>
													<td class="text-center">
														<?= (isset($priorizacion['Etario'.$i.'_'.$value->CODIGO][$semana])) ? $priorizacion['Etario'.$i.'_'.$value->CODIGO][$semana] : ""; ?>
													</td>
												<?php } ?>
											<td class="text-center"><?= $totalesPriorizacion['Etario'.$i.'_'.$value->CODIGO]; ?></td>
										</tr>
									<?php } ?>
									<tr>
										<th>Total <?= $value->CODIGO; ?></th>
											<?php foreach ($semanas as $semana){ ?>
												<th class="text-center"><?= isset($priorizacion[$value->CODIGO][$semana]) ? $priorizacion[$value->CODIGO][$semana] : ""; ?></th>
											<?php } ?>
										<th class="text-center"><?= $totalesPriorizacion[$value->CODIGO]; ?></th>
									</tr>	
								<?php endforeach ?>
          			...
          			<div class="clients-list">
            			<ul class="nav nav-tabs">
							<?php
								...
								foreach ($semanas as $semana){
							?>
								<li class="<?php if($auxIndice == 1){ echo ' active '; } ?> ">
									<a data-toggle="tab" href="#tab-<?= $semana; ?>"><i class="fa fa-child"></i> Sem <?= $semana; ?></a>
								</li>
							<?php
									$auxIndice++;
								}
