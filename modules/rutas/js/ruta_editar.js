...
function actualizarRuta(){
  console.log('Se va a actualizar la ruta');
  ...
  if(bandera == 0){
    ...
    $.ajax({
      type: "POST",
      url: "functions/fn_ruta_actualizar.php",
      data: datos,
      beforeSend: function(){
      ...
      success: function(data){
        if(data == 1){
          alert('La ruta se ha actualizado con éxito.');
...
function agregarItemsIniciales(){
  itemsActuales = [];
  $('#selectVarios').prop( "checked", false );
  ...
  if(bandera == 0){
    ...
    $.ajax({
      type: "POST",
      url: "functions/fn_rutas_agregar_items_inicio.php",
      data: datos,
      beforeSend: function(){
