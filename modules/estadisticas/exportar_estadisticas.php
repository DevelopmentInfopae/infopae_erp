...
require_once 'functions/fn_estadisticas_functions.php';
...
if (isset($_GET['semana'])) {
   ...
   $sheet->getStyle("D2:P5")->applyFromArray($titulos);
   ...
   foreach ($totalesSemanas as $mes => $semanas) {
	   $sheet->setCellValue("B".$numFila, buscar_meses_nombre(intval($mes)));
   ...
   // inicio seccion totales por complemento alimentario 
   $numFila = $numFila+2;
   ...
   foreach ($totalesComplementos as $mes => $complementos) {
	   $Letra = "B";
	   $sheet->setCellValue($Letra.$numFila, buscar_meses_nombre(intval($mes)));
   ...
   // inicio tipo complemento por semana
   $numFila = $numFila+2;
   ...
   // exit(var_dump($dias));
   $diasTxt = "del ".$dias[0]." de ".buscar_meses_nombre(intval($mesDeDia[$dias[0]]))." al ".$dias[sizeof($dias)-1]." de ".buscar_meses_nombre(intval($mesDeDia[$dias[sizeof($dias)-1]]));
   ...
   $chart->setBottomRightPosition('K'.$numFila);
   $sheet->addChart($chart);
   // totales por complementos y grupos etarios
   $numFila += 2;
   ...
   foreach ($etarios as $key => $etario) {
      foreach ($tipoComplementos as $key => $value) {
			   ...
			   while ($Complementos = $resComplementos->fetch_assoc()) {
                  ...
                  if (is_null($Complementos['tipo_complem'])) {
                     continue;
					   ...
					   if (!isset($diasComplementos[$diaFecha."-".buscar_meses_nombre(intval($mesDeDia[$diaFecha]))][$Complementos['tipo_complem']]) && isset($Complementos[$diaFecha])) {
						   $diasComplementos[$diaFecha."-".buscar_meses_nombre(intval($mesDeDia[$diaFecha]))][$Complementos['tipo_complem']] = $Complementos[$diaFecha];
					   } else if (!isset($diasComplementos[$diaFecha."-".buscar_meses_nombre(intval($mesDeDia[$diaFecha]))][$Complementos['tipo_complem']])) {
						   $diasComplementos[$diaFecha."-".buscar_meses_nombre(intval($mesDeDia[$diaFecha]))][$Complementos['tipo_complem']] = 0;
					   }
				   }
			   }
		   }
   ...
   $sheet->getStyle('B'.$numFila.':P'.$numFila)->applyFromArray($titulos);
   $numFila += 2;
   $consValores = "SELECT 'ValorContrato' AS Concepto, ValorContrato 
                     FROM parametros
                     UNION
