...
var datos = $('#formFichaTecnica').serialize();
    datos = datos+"&tipoProducto="+$('#tipoProducto').val();
    console.log(datos);
$.ajax({
    type: "POST",
...
var datos = $('#formCalyNut').serialize();
    datos = datos+"&idProducto="+$('#idProducto').val();
$.ajax({
    type: "POST",
...
function eliminarFTDet(){
  ...
  success: function(data){
    if (data == "1") {
      ...
      Command: toastr.warning("El menú al que está relacionado el producto, está despachado.", "No se puede desasociar el producto.", {onHidden : function(){$('#loader').fadeOut();}})
    } else {
...
function exportarMenu(codigo){
  $('#idProductoExportar').val(codigo);
  $('#exportar_menu').submit();
}