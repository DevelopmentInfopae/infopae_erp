...
$validacionOrdenMayor = "SELECT id FROM prioridad_caracterizacion WHERE orden = (SELECT max(orden) FROM prioridad_caracterizacion);";
$respuestaValidacionOrdenMayor = $Link->query($validacionOrdenMayor) or die('Error al consultar el id del orden mayor:'.mysqli_error($Link));
if ($respuestaValidacionOrdenMayor->num_rows > 0) {
	...
	if ($id == $dataValidacionOrdenMayor['id']) {
		$respuestaAJAX = [
			'estado' => 0,
			'mensaje' => 'Este es el orden mayor existente'
		];
		exit(json_encode($respuestaAJAX));	
	}
}

// secuencia para subir el orden origen
...
if ($respuestaOrden->num_rows > 0) {
	...
	$ordenAumentado = ++$ordenActual;
	// exit(var_dump($consultaOrden));
	// sentencia updata para subir el numero del orden 
	$sentenciaUpdateSubir = "UPDATE prioridad_caracterizacion SET orden = '".$ordenAumentado."' WHERE id = '".$id."';";
	...
	if ($respuestaUpdateSubir) {
		$consultaBitacora = "INSERT INTO bitacora (fecha, usuario, tipo_accion, observacion) VALUES ('" . date("Y-m-d H-i-s") . "', '" . $_SESSION["idUsuario"] . "', '72', 'Se subio el orden prioridad caracterizacion: <strong>".$descripcion."</strong>')";
   		$Link->query($consultaBitacora) or die ('Unable to execute query. '. mysqli_error($Link));

   		// secuencia para bajar si existe el orden si es igual al orden aumentado 
		$consultaOrdenExistente = "SELECT id, orden, descripcion FROM prioridad_caracterizacion WHERE orden = '" .$ordenAumentado. "' AND id !='" . $id ."';";
		...
		if ($respuestaOrdenExistente->num_rows > 0) {
			...
			$ordenDisminuido = --$ordenE;
			$sentenciaUpdateBajar = "UPDATE prioridad_caracterizacion SET orden = '" .$ordenDisminuido. "' WHERE id = '" .$idE. "';";
			...
			if ($respuestaUpdateBajar) {
				$consultaBitacora = "INSERT INTO bitacora (fecha, usuario, tipo_accion, observacion) VALUES ('" . date("Y-m-d H-i-s") . "', '" . $_SESSION["idUsuario"] . "', '73', 'Se bajo el orden prioridad caracterizacion: <strong>".$descripcionE."</strong>')";
   				$Link->query($consultaBitacora) or die ('Unable to execute query. '. mysqli_error($Link));
			}
		}
  		...
  		$respuestaAJAX = [
  		'estado' => 1,
  		'mensaje' => 'El Orden prioridad caracterización se aumentó exitosamente'
  		];
  		exit (json_encode($respuestaAJAX));
	}
}
