	...
	<div class="wrapper wrapper-content  animated fadeInRight">
		<div class="row">
			<div class="col-sm-12">
				<div class="sedes">
					<?php
						$consulta = "SELECT DISTINCT(s.cod_sede), s.nom_sede, s.cod_inst, s.nom_inst FROM sedes$periodoActual s WHERE s.cod_sede IN (SELECT distinct cod_sede  FROM dispositivos)"; 
						if($municipio != ""){
						...
						if($resultado->num_rows >= 1){
							while($row = $resultado->fetch_assoc()){
								...
								if($resultado2 !== false && $resultado2->num_rows > 0){
									...
									$sellado = $row2['estado'];
									$consulta3 = "SELECT DISTINCT(s.cod_sede), 
														s.nom_sede, 
														s.cod_inst, 
														s.nom_inst, 
														a.estado AS sellado , 
									...
									$consulta4 = "SELECT f.cod_grado, 
														...
														WHERE f2.cod_sede = $codSede AND a.consumio IS not NULL and a.dia = \"$dia\" AND f2.nom_grupo = f.nom_grupo 
														GROUP BY f2.nom_grupo ) AS entregado 
													FROM focalizacion$semanaActual f 
													left join grados g on g.id = f.cod_grado 
													WHERE f.cod_sede = $codSede 
													GROUP BY nom_grupo ";
								}else{
									$consulta3 = "	SELECT DISTINCT(s.cod_sede), 
														s.nom_sede, 
														...
														s.nom_inst , 
														(select count(DISTINCT f.num_doc) AS total 
														from focalizacion$semanaActual f WHERE f.cod_sede = s.cod_sede ) AS total ,
														(SELECT SUM(t.entregas) AS entregado 
													FROM (	select IF(COUNT(f2.id)>2,1, COUNT(f2.id)) AS entregas, f2.*
															from biometria_reg br 
															left join  biometria b on (br.usr_dispositivo_id=b.id_bioest and br.dispositivo_id=b.id_dispositivo)
									...
									$consulta4 = "	SELECT f.cod_grado, 
														...
														FROM (	select IF(COUNT(f2.id)>2,1, COUNT(f2.id)) AS entregas, f2.*
																...
																WHERE b.cod_sede = \"$codSede\" AND  YEAR(br.fecha) = $anno AND MONTH(br.fecha) = $mes AND DAY(br.fecha) = $dia
																GROUP BY f2.id) AS t 
														WHERE t.cod_sede = $codSede AND t.nom_grupo = f.nom_grupo ) AS entregado 
													FROM focalizacion$semanaActual f 
													left join grados g on g.id = f.cod_grado 
													WHERE f.cod_sede = $codSede 
													GROUP BY nom_grupo ";
...
<script src="<?= $baseUrl; ?>/modules/asistencias/js/control_biometrico.js?v=<?= $cacheBusting; ?>"></script>
