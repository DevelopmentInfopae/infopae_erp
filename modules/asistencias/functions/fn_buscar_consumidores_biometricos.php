...
if($resultado->num_rows < 1){
	$banderaCamposLector++;
	$consulta = " insert into asistencia_enc$mes$anno ( mes, semana, dia, estado, cod_sede, complemento ) values ( \"$mes\", \"$semanaActual\", \"$dia\", \"1\", \"$sede\", \"$complemento\" ) ";
	// echo "$consulta";
	...
	if($resultado->num_rows > 0){
		$consulta3 = " insert into asistencia_det$mes$anno (tipo_doc, num_doc, fecha, mes, semana, dia, asistencia, id_usuario, repite, consumio, repitio, complemento) values ";
		$aux = 0;
		while($row = $resultado->fetch_assoc()){
			...
			if($entregas > 1){ $consulta3 .= " \"1\", "; }else{ $consulta3 .= " \"0\", "; }
			$consulta3 .= " \"1\", ";
			if($entregas > 1){ $consulta3 .= " \"1\", "; }else{ $consulta3 .= " \"0\", "; }
	...
	$consulta = "SELECT 0 as entregas, f3.tipo_doc, f3.num_doc FROM focalizacion$semanaActual f3 where f3.cod_sede = \"$sede\" and (f3.tipo_doc, f3.num_doc) not in (SELECT f2.tipo_doc, f2.num_doc FROM focalizacion$semanaActual f2 LEFT JOIN biometria b ON f2.tipo_doc = b.tipo_doc AND f2.num_doc = b.num_doc LEFT JOIN biometria_reg br ON b.id_dispositivo = br.dispositivo_id AND b.id_bioest = br.usr_dispositivo_id WHERE id_dispositivo IS NOT NULL AND id_bioest IS NOT NULL AND year(br.fecha) = $anno4d AND month(br.fecha) = $mes AND day(br.fecha) = $dia AND f2.cod_sede = \"$sede\" GROUP BY f2.num_doc ) "; 
	$resultado = $Link->query($consulta);
	if($resultado->num_rows > 0){
		$consulta3 = " insert into asistencia_det$mes$anno (tipo_doc, num_doc, fecha, mes, semana, dia, asistencia, id_usuario, repite, consumio, repitio, complemento) values ";
		$aux = 0;
